...
package object element {
  ...
  implicit class ZinteractWebElement(element: WebElement) {
    ...
    /** Click this element. If this causes a new page to load, you should discard all references to this element and any
      * further operations performed on this element will throw a StaleElementReferenceException.
      *
      * Note that if click() is done by sending a native event (which is the default on most browsers/platforms) then
      * the method will _not_ wait for the next page to load and the caller should verify that themselves.
      *
      * There are some preconditions for an element to be clicked. The element must be visible and it must have a height
    ...
    /** If this element is a text entry element, this will clear the value. Has no effect on other elements. Text entry
      ...
      *
      * Note that the events fired by this event may not be as you'd expect. In particular, we don't fire any keyboard
      * or mouse events.
      */
    ...
    /** Gets the value of the given attribute of the element. Will return the current value, even if this has been
      ...
      *
      * <p>More exactly, this method will return the value of the property with the given name, if it exists. If it does
      * not, then the value of the attribute with the given name is returned. If neither exists, null is returned.
      *
      ...
      *
      * <p>async, autofocus, autoplay, checked, compact, complete, controls, declare, defaultchecked, defaultselected,
      ...
      *
      * <ul> <li>If the given name is "class", the "className" property is returned. <li>If the given name is
      * "readonly", the "readOnly" property is returned. </ul>
      *
      * <i>Note:</i> The reason for this behavior is that users frequently confuse attributes and properties. If you
      * need to do something more precise, e.g., refer to an attribute even when a property of the same name exists,
      * then you should evaluate Javascript to obtain the result you desire.
      */
    ...
    /** Gets the value of a given CSS property. Color values should be returned as rgba strings, so, for example if the
      * "background-color" property is set as "green" in the HTML source, the returned value will be "rgba(0, 255, 0,
      * 1)".
      *
      * Note that shorthand CSS properties (e.g. background, font, border, border-top, margin, margin-top, padding,
      * padding-top, list-style, outline, pause, cue) are not returned, in accordance with the
    ...
    /** Returns if the element currently enabled or not. This will generally return true for everything but disabled
      * input elements.
      *
      * @return
  ...
  /** Click this element. If this causes a new page to load, you should discard all references to this element and any
    * further operations performed on this element will throw a StaleElementReferenceException.
    *
    * Note that if click() is done by sending a native event (which is the default on most browsers/platforms) then the
    * method will _not_ wait for the next page to load and the caller should verify that themselves.
    *
    * There are some preconditions for an element to be clicked. The element must be visible and it must have a height
    * and width greater then 0.
    */
  ...
  /** If this element is a text entry element, this will clear the value. Has no effect on other elements. Text entry
    * elements are INPUT and TEXTAREA elements.
    *
    * Note that the events fired by this event may not be as you'd expect. In particular, we don't fire any keyboard or
    * mouse events.
    */
  ...
  /** Gets the value of the given attribute of the element. Will return the current value, even if this has been
    ...
    * <p>Finally, the following commonly mis-capitalized attribute/property names are evaluated as expected:
    *
    * <ul> <li>If the given name is "class", the "className" property is returned. <li>If the given name is "readonly",
    * the "readOnly" property is returned. </ul>
    *
    * <i>Note:</i> The reason for this behavior is that users frequently confuse attributes and properties. If you need
    * to do something more precise, e.g., refer to an attribute even when a property of the same name exists, then you
